<!-- resources/views/tyre/refilling/create.blade.php -->
...
<div class="page-header d-flex justify-content-between align-items-center">
    <div class="page-title">
        ...
        <h6>Send tyres for refilling/retreading</h6>
    </div>
    <div class="page-btn">
...
<form action="{{ route('tyre.refilling.store') }}" method="POST" id="refillingForm">
    @csrf
    <div class="card">
            ...
            <div class="row">
                ...
                <div class="col-lg-12">
                    <div class="form-group">
                        ...
                        <select name="tyre_ids[]" class="form-control select2" id="tyreSelect" multiple required>
                            @foreach($availableTyres as $tyre)
                            <option value="{{ $tyre->id }}" {{ isset($selectedTyreId) && $selectedTyreId == $tyre->id ? 'selected' : '' }}>
                                {{ $tyre->serial_number }} - {{ $tyre->brand }} ({{ $tyre->size }}) - {{ ucfirst($tyre->status) }}
                            </option>
                            @endforeach
                        </select>
                        <small class="text-muted">You can select multiple tyres for this order</small>
                    </div>
                </div>
                <div class="col-lg-12">
                    <div class="form-group">
...
<script>
    $(document).ready(function() {
        ...
        // Initialize Select2 for Tyres dropdown with search (multiple)
        $('#tyreSelect').select2({
            placeholder: "Search and select tyres...",
            allowClear: true,
            width: '100%',
            closeOnSelect: false
        });
